...
| `--no-web-search` | `false` | Disable web search for this request |
...
Pass `--no-web-search` to disable web search for this follow-up request.
...
`web_search` defaults to `true` when omitted. Set it to `false` to disable web
search by default; `start --no-web-search` and `send --no-web-search` always
disable it for that request.
